  ...
  class GetPlatformLimitResponseEbaySellingCap
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CrosslyBuyer::GetPlatformLimitResponseEbaySellingCap` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CrosslyBuyer::GetPlatformLimitResponseEbaySellingCap`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'quantity_used')
        self.quantity_used = attributes[:'quantity_used']
      end
    ...
    def list_invalid_properties
      ...
      if @fetched_at.nil?
        invalid_properties.push('invalid value for "fetched_at", fetched_at cannot be nil.')
      end
    ...
    def hash
      [amount_limit, amount_used, quantity_limit, quantity_used, currency, fetched_at].hash
    end
