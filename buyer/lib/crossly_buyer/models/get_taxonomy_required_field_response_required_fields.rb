  ...
  class GetTaxonomyRequiredFieldResponseRequiredFields
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CrosslyBuyer::GetTaxonomyRequiredFieldResponseRequiredFields` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CrosslyBuyer::GetTaxonomyRequiredFieldResponseRequiredFields`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      data_type_validator = EnumAttributeValidator.new('String', ["STRING", "NUMBER", "BOOLEAN"])
    ...
    def hash
      [key, label, master, required, data_type, has_enum_values, enum_values, hint].hash
    end
