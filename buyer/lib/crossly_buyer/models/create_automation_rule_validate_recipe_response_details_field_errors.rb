  ...
  class CreateAutomationRuleValidateRecipeResponseDetailsFieldErrors
    ...
    def self.openapi_types
      {
        ...
        :'schema' => :'Array<String>',
        :'schema_version' => :'Array<String>',
        :'exported_at' => :'Array<String>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CrosslyBuyer::CreateAutomationRuleValidateRecipeResponseDetailsFieldErrors` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CrosslyBuyer::CreateAutomationRuleValidateRecipeResponseDetailsFieldErrors`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'schema_version')
        if (value = attributes[:'schema_version']).is_a?(Array)
          self.schema_version = value
        end
      end

      if attributes.key?(:'exported_at')
        if (value = attributes[:'exported_at']).is_a?(Array)
    ...
    def hash
      [name, condition, action, metadata, description, platforms, trigger, schema, schema_version, exported_at, recipes].hash
    end
