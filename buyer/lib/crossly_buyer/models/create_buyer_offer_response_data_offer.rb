  ...
  class CreateBuyerOfferResponseDataOffer
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CrosslyBuyer::CreateBuyerOfferResponseDataOffer` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CrosslyBuyer::CreateBuyerOfferResponseDataOffer`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @buyer_user_id.nil?
        invalid_properties.push('invalid value for "buyer_user_id", buyer_user_id cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @buyer_user_id.nil?
    ...
    def hash
      [id, status, platform_listing_id, buyer_user_id, amount_cents, message].hash
    end
