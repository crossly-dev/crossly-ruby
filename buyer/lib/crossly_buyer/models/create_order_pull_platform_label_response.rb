  ...
  class CreateOrderPullPlatformLabelResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CrosslyBuyer::CreateOrderPullPlatformLabelResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CrosslyBuyer::CreateOrderPullPlatformLabelResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      provenance_validator = EnumAttributeValidator.new('String', ["verified", "provisional"])
      return false unless provenance_validator.valid?(@provenance)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] provenance Object to be assigned
    def provenance=(provenance)
      validator = EnumAttributeValidator.new('String', ["verified", "provisional"])
      unless validator.valid?(provenance)
        fail ArgumentError, "invalid value for \"provenance\", must be one of #{validator.allowable_values}."
      end
      @provenance = provenance
    end
    ...
    def hash
      [url, persisted, expires_in_seconds, provenance].hash
    end
