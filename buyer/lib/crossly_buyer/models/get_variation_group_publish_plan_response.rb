  ...
  class GetVariationGroupPublishPlanResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CrosslyBuyer::GetVariationGroupPublishPlanResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CrosslyBuyer::GetVariationGroupPublishPlanResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'axis_name')
        self.axis_name = attributes[:'axis_name']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @axis_name.nil?
        invalid_properties.push('invalid value for "axis_name", axis_name cannot be nil.')
      end
    ...
    def hash
      [plans, total_listings, member_count, axis_name].hash
    end
