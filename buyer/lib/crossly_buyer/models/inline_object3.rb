  ...
  class InlineObject3
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CrosslyBuyer::InlineObject3` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CrosslyBuyer::InlineObject3`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @amount_cents < 100
        invalid_properties.push('invalid value for "amount_cents", must be greater than or equal to 100.')
      end
    ...
    def valid?
      ...
      return false if @amount_cents < 100
    ...
    def amount_cents=(amount_cents)
      ...
      if amount_cents < 100
        fail ArgumentError, 'invalid value for "amount_cents", must be greater than or equal to 100.'
      end
    ...
    def hash
      [listing_slug, amount_cents, message].hash
    end
